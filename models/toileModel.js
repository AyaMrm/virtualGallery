...
const getToileById =(idToile, callback)=>{
    const query = 'SELCECT * FROM toile WHERE idToile =?';
    db.query(query, [idToile], callback);
}

const deleteToile =(idToile, callback)=>{
    const query = 'DELECTE FROM toile WHERE idToile =?';
    db.query(query, [idToile], callback);
}
...
const updateToile = (idToile, toile, callback)=>{
    ...
    const values = [toile.titre, toile.dateCreation, toile.technique, toile.support, toile.style, toile.idArtiste, toile.idMusee, toile.idToile];
    db.query(query, values, callback);
}
...
const getToileByIdArtiste =(idArtiste, callabck)=>{
    const query = 'SELCET * FROM toile WHERE idArtiste=?';
    db.query(query, [idArtiste], callaback);
}
